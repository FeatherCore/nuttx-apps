...
#ifdef __NuttX__
static void hostapd_cli_reset_for_builtin(void)
{
	optind = 1;
	optarg = NULL;
	optopt = 0;
	...
	ctrl_iface_dir = CONFIG_CTRL_IFACE_DIR;
	client_socket_dir = NULL;
	ctrl_ifname = NULL;
	pid_file = NULL;
	action_file = NULL;
	ping_interval = 5;
...
static int hostapd_cli_cmd_file_arg(struct wpa_ctrl *ctrl, const char *cmd,
				    int min_args, int argc, char *argv[])
{
	char *file;
	char *arg;
	...
	file = os_readfile(argv[0] + 1, &len);
	if (!file) {
		printf("Could not read %s argument file: %s\n", cmd,
		       argv[0] + 1);
		return -1;
	}
	...
	os_memcpy(arg, file, len);
	arg[len] = '\0';
	os_free(file);

	file_argv[0] = arg;
	ret = hostapd_cli_cmd(ctrl, cmd, min_args, 1, file_argv);
	os_free(arg);
	return ret;
}
...
static int hostapd_cli_cmd_dpp_qr_code(struct wpa_ctrl *ctrl, int argc,
				       char *argv[])
{
	return hostapd_cli_cmd_file_arg(ctrl, "DPP_QR_CODE", 1, argc, argv);
}
...
static int hostapd_cli_cmd_dpp_auth_init(struct wpa_ctrl *ctrl, int argc,
					 char *argv[])
{
	return hostapd_cli_cmd_file_arg(ctrl, "DPP_AUTH_INIT", 1, argc, argv);
}
...
#ifdef __NuttX__
	/*
	 * Built-in NuttX applications run in one flat address space. A one-shot
	 * hostapd_cli command must not reinitialize or destroy the global eloop
	 * while a background hostapd task is using it.
	 */
	use_eloop = interactive || action_file || daemonize || reconnect;
	if (use_eloop && eloop_init())
		return -1;
#else
	if (eloop_init())
		return -1;
...
#ifdef __NuttX__
	if (use_eloop)
		eloop_register_signal_terminate(hostapd_cli_eloop_terminate,
						NULL);
#else
