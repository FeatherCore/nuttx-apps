				 ...
				 struct nlattr *tb)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211-debug: hostapd supp_cmds auth=%u connect=%u",
		   NL80211_CMD_AUTHENTICATE, NL80211_CMD_CONNECT);

	if (tb == NULL) {
		wpa_printf(MSG_DEBUG, "nl80211-debug: hostapd supp_cmds attr missing");
		return;
	}

	nla_for_each_nested(nl_cmd, tb, i) {
		wpa_printf(MSG_DEBUG, "nl80211-debug: hostapd supp_cmd=%u",
			   nla_get_u32(nl_cmd));
