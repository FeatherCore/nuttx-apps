		       ...
		       u8 *pos, size_t left)
{
	...
	wpa_printf(MSG_DEBUG,
		   "FILS: AssocReq decrypt bounds frame_len=%zu capab_off=%td ie_start_off=%td session_off=%td crypt_off=%td aad4=%zu crypt_len=%td kek_len=%zu snonce=%02x%02x anonce=%02x%02x",
		   frame_len, aad[4] - (const u8 *) mgmt,
		   ie_start - (const u8 *) mgmt, session - (const u8 *) mgmt,
		   crypt - (const u8 *) mgmt, aad_len[4], end - crypt,
		   sm->PTK.kek_len, sm->SNonce[0], sm->SNonce[1],
		   sm->ANonce[0], sm->ANonce[1]);
