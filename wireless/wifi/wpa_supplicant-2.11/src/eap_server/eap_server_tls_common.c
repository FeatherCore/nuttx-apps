...
int eap_server_tls_phase1(struct eap_sm *sm, struct eap_ssl_data *data)
{
	...
	if (data->tls_out == NULL) {
		wpa_printf(MSG_INFO,
			   "SSL: TLS processing failed (failed=%d read_alerts=%d write_alerts=%d)",
			   tls_connection_get_failed(sm->cfg->ssl_ctx,
						     data->conn),
			   tls_connection_get_read_alerts(sm->cfg->ssl_ctx,
							  data->conn),
			   tls_connection_get_write_alerts(sm->cfg->ssl_ctx,
							   data->conn));
		return -1;
	}
	if (tls_connection_get_failed(sm->cfg->ssl_ctx, data->conn)) {
		...
		wpa_printf(MSG_DEBUG,
			   "SSL: Failed - tls_out available to report error (read_alerts=%d write_alerts=%d)",
			   tls_connection_get_read_alerts(sm->cfg->ssl_ctx,
							  data->conn),
			   tls_connection_get_write_alerts(sm->cfg->ssl_ctx,
							   data->conn));
