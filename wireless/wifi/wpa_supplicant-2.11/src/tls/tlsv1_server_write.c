...
static u8 * tls_send_server_hello(struct tlsv1_server *conn, size_t *out_len)
{
	...
	*out_len = 0;
	...
	/*
	 * The fixed portion has to cover ServerHello, ServerKeyExchange,
	 * CertificateRequest, and HelloDone. RSA3072 signatures plus DHE
	 * parameters can exceed the old 1000-byte allowance even when the
	 * certificate chain itself is accounted separately.
	 */
	msglen = 2048 + tls_server_cert_chain_der_len(conn) + ocsp_resp_len;
