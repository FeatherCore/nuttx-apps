			...
			void *eloop_data, void *user_data)
{
	...
	ret = eloop_sock_table_add_sock(table, sock, handler,
					eloop_data, user_data);
	return ret;
}
...
#ifdef __NuttX__
			/*
			 * Some NuttX socket backends can report an immediate
			 * zero-result wait while initialization is still settling.
			 * Yield briefly so background daemons do not starve NSH
			 * commands in sim builds without round-robin scheduling.
			 */
			usleep(1000);
#endif /* __NuttX__ */
			continue;
...
#ifdef __NuttX__
		/*
		 * In the sim build, the imported nl80211/l2_packet sockets can
		 * stay continuously readable during bring-up. Yield each handled
		 * batch so NSH and wpa_cli can run while the daemon is active.
		 */
		usleep(1000);
#endif /* __NuttX__ */
	}
