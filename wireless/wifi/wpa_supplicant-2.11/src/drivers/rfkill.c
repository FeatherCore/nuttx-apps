...
#ifdef __NuttX__
	/* NuttX does not expose Linux sysfs, so keep the rfkill semantics as an
	 * in-process WLAN radio state object. ESP-Hosted NG on STM32H7S7 has no
	 * separate hardware rfkill input wired at the moment, so the initial
	 * state is unblocked. If a board later provides a hard/soft block source,
	 * this object can feed rfkill_receive()-equivalent events into the same
	 * blocked/unblocked callbacks.
	 */
	rfkill->cfg = cfg;
	rfkill->blocked = 0;
	rfkill->idx = 0;
	wpa_printf(MSG_DEBUG,
		   "rfkill: NuttX WLAN state ifname=%s idx=%u soft=0 hard=0",
		   cfg->ifname, (unsigned int) rfkill->idx);
	return rfkill;
#endif /* __NuttX__ */
