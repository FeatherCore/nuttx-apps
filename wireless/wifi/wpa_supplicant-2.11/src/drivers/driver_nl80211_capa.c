...
static u32 get_nl80211_protocol_features(struct wpa_driver_nl80211_data *drv)
{
	...
	nuttx_hwsim_debugf("hwsim-debug: get_nl80211_protocol_features failed\n");
	return 0;
}
...
static int wiphy_info_handler(struct nl_msg *msg, void *arg)
{
	...
	if (tb[NL80211_ATTR_VENDOR_DATA]) {
		...
		nla_for_each_nested(nl, tb[NL80211_ATTR_VENDOR_DATA], rem_vendor) {
	...
	if (tb[NL80211_ATTR_VENDOR_EVENTS]) {
		struct nlattr *nl;
		int rem_vendor_event;

		nla_for_each_nested(nl, tb[NL80211_ATTR_VENDOR_EVENTS], rem_vendor_event) {
			struct nl80211_vendor_cmd_info *vinfo;
			if (nla_len(nl) != sizeof(*vinfo)) {
				wpa_printf(MSG_DEBUG, "nl80211: Unexpected vendor data info");
				continue;
				       ...
				       struct wiphy_info_data *info)
{
	...
	nuttx_hwsim_debugf("hwsim-debug: get_info protocol_features=0x%x\n", feat);
	...
	if (!msg || nla_put_flag(msg, NL80211_ATTR_SPLIT_WIPHY_DUMP)) {
		nuttx_hwsim_debugf("hwsim-debug: get_info build GET_WIPHY failed msg=%p\n",
		       msg);
		nlmsg_free(msg);
		return -1;
	}

	nuttx_hwsim_debugf("hwsim-debug: get_info send GET_WIPHY flags=0x%x\n", flags);
	if (send_and_recv_resp(drv, msg, wiphy_info_handler, info)) {
		nuttx_hwsim_debugf("hwsim-debug: get_info GET_WIPHY failed\n");
		return -1;
	}
	nuttx_hwsim_debugf("hwsim-debug: get_info GET_WIPHY ok error=%u auth=%u connect=%u\n",
	       info->error, info->auth_supported, info->connect_supported);
...
#ifdef __NuttX__
	/*
	 * The imported mac80211/cfg80211 stack can expose the nl80211
	 * scheduled-scan command through cfg80211 while the current NuttX
	 * lower-half has no sched_scan_start/stop implementation. Avoid
	 * entering that unsupported offload path during supplicant bring-up.
	 *
	 * Likewise, keep AP/GO EAPOL on PF_PACKET for now. The NuttX packet
	 * data path handles EAPOL frames, while the nl80211 control-port TX
	 * command is not complete enough yet for P2P group formation.
	 */
	drv->capa.sched_scan_supported = 0;
	drv->capa.max_sched_scan_ssids = 0;
	drv->capa.max_sched_scan_plans = 0;
	drv->capa.max_sched_scan_plan_interval = 0;
	drv->capa.max_sched_scan_plan_iterations = 0;
	drv->capa.mac_addr_rand_sched_scan_supported = 0;
			  ...
			  struct nlattr *tb_freq[])
{
	...
	chan->dfs_cac_ms = 0;
	if (chan->freq < 2000) {
		chan->chan = (chan->freq - 902) * 2 +
			(chan->freq_offset ? 1 : 0);
	} else if (ieee80211_freq_to_chan(chan->freq, &channel) !=
		   NUM_HOSTAPD_MODES) {
		chan->chan = channel;
	} else {
		wpa_printf(MSG_DEBUG,
