...
static struct nl_sock * nl_create_handle(struct nl_cb *cb, const char *dbg)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Create generic netlink handle (%s)",
		   dbg);
	...
	wpa_printf(MSG_DEBUG, "nl80211: Generic netlink handle ready (%s)",
		   dbg);
...
static int ack_handler_wait_valid(struct nl_msg *msg, void *arg)
{
	struct nl80211_ack_wait_arg *wait = arg;

	if (!*wait->valid_seen) {
		*wait->ack_seen = 1;
		nuttx_hwsim_debugf("hwsim-debug: ACK before valid, keep receiving\n");
		return NL_SKIP;
	}
			 ...
			 void *arg)
{
	...
	nuttx_hwsim_debugf("hwsim-debug: error handler seq=%u orig_seq=%u error=%d\n",
	       nlh->nlmsg_seq, orig_nlh->nlmsg_seq, err->error);
...
static int nl80211_seq_check(struct nl_msg *msg, void *arg)
{
	unsigned int *seq = arg;
	struct nlmsghdr *nlh = nlmsg_hdr(msg);

	if (nlh->nlmsg_seq != *seq) {
		nuttx_hwsim_debugf("hwsim-debug: skip stale nlmsg seq=%u want=%u type=%u\n",
		       nlh->nlmsg_seq, *seq, nlh->nlmsg_type);
		return NL_SKIP;
	}
...
static int valid_handler_debug(struct nl_msg *msg, void *arg)
{
	struct nl80211_valid_cb *valid = arg;
#ifdef CONFIG_WL_NUTTX_HWSIM_DEBUG
	struct nlmsghdr *nlh = nlmsg_hdr(msg);

	nuttx_hwsim_debugf("hwsim-debug: valid msg type=%u len=%u flags=0x%x seq=%u\n",
	...
	if (valid->handler)
	{
		...
		ret = valid->handler(msg, valid->arg);
		if (valid->valid_seen)
			*valid->valid_seen = 1;
		if (valid->ack_seen && *valid->ack_seen && valid->err) {
			*valid->err = 0;
			return NL_STOP;
		}
		  ...
		  struct nl80211_err_info *err_info)
{
	...
	int opt;
	struct nlmsghdr *dbg_nlh;
	struct genlmsghdr *dbg_gnlh;
	unsigned int dbg_type;
	unsigned int dbg_cmd;
	unsigned int dbg_flags;
#ifdef CONFIG_WL_NUTTX_HWSIM_DEBUG
#endif
	struct nl80211_valid_cb valid_debug;
	struct nl80211_ack_wait_arg ack_wait;
	int valid_seen = 0;
	int ack_seen = 0;
	...
	dbg_nlh = nlmsg_hdr(msg);
	dbg_gnlh = nlmsg_data(dbg_nlh);
	dbg_type = dbg_nlh->nlmsg_type;
	dbg_cmd = dbg_gnlh->cmd;
	dbg_flags = dbg_nlh->nlmsg_flags;

	wpa_printf(MSG_DEBUG,
		   "nl80211: Send netlink command type=%u cmd=%u flags=0x%x",
		   dbg_type, dbg_cmd, dbg_flags);

#ifdef CONFIG_WL_NUTTX_HWSIM_DEBUG
	nuttx_hwsim_debugf("hwsim-debug: send_and_recv send type=%u cmd=%u flags=0x%x\n",
	       dbg_type, dbg_cmd, dbg_flags);
#endif
	...
	req_seq = nlmsg_hdr(msg)->nlmsg_seq;
	wpa_printf(MSG_DEBUG,
		   "nl80211: Waiting for netlink command cmd=%u seq=%u",
		   dbg_cmd, req_seq);
	err.err = 1;
	err.orig_msg = msg;
	...
	} else if (valid_handler) {
		ack_wait.err = &err.err;
		ack_wait.valid_seen = &valid_seen;
		ack_wait.ack_seen = &ack_seen;
		nl_cb_set(cb, NL_CB_ACK, NL_CB_CUSTOM,
			  ack_handler_wait_valid, &ack_wait);
	} else {
		nl_cb_set(cb, NL_CB_ACK, NL_CB_CUSTOM, ack_handler, &err);
	}

	if (valid_handler) {
		valid_debug.handler = valid_handler;
		valid_debug.arg = valid_data;
		valid_debug.err = &err.err;
		valid_debug.valid_seen = &valid_seen;
		valid_debug.ack_seen = &ack_seen;
		nl_cb_set(cb, NL_CB_VALID, NL_CB_CUSTOM,
			  valid_handler_debug, &valid_debug);
	}
...
out:
	...
	nlmsg_free(msg);
	wpa_printf(MSG_DEBUG,
		   "nl80211: Netlink command cmd=%u seq=%u completed ret=%d",
		   dbg_cmd, req_seq, err.err);
#ifdef CONFIG_WL_NUTTX_HWSIM_DEBUG
	nuttx_hwsim_debugf("hwsim-debug: send_and_recv done type=%u cmd=%u ret=%d\n",
	       dbg_type, dbg_cmd, err.err);
#endif
	return err.err;
}
...
struct nl_msg * nl80211_cmd_msg_alloc(struct i802_bss *bss, int flags,
...
static int netdev_info_handler(struct nl_msg *msg, void *arg)
{
	...
	nuttx_hwsim_debugf("hwsim-debug: netdev_info cmd=%u attrlen=%d\n",
	       gnlh->cmd, genlmsg_attrlen(gnlh, 0));
	nla_for_each_attr(attr, genlmsg_attrdata(gnlh, 0),
			  genlmsg_attrlen(gnlh, 0), rem) {
		nuttx_hwsim_debugf("hwsim-debug: netdev_info attr type=%u len=%u\n",
...
int nl80211_get_wiphy_index(struct i802_bss *bss)
{
	...
	if (send_and_recv_resp(bss->drv, msg, netdev_info_handler, &data) == 0) {
		if (data.wiphy_idx < 0 && bss->drv->phyname[0]) {
			nuttx_hwsim_debugf("hwsim-debug: get_wiphy_index fallback drv idx=%u phy=%s\n",
			       bss->drv->wiphy_idx, bss->drv->phyname);
...
static int wpa_driver_nl80211_init_nl_global(struct nl80211_global *global)
{
	...
	nl_cache_free(cache);
	wpa_printf(MSG_DEBUG, "nl80211: Generic netlink cache initialized");
	...
	wpa_printf(MSG_DEBUG, "nl80211: Register event netlink socket");
	nl80211_register_eloop_read(&global->nl_event,
				    wpa_driver_nl80211_event_receive,
				    global->nl_cb, 0);
	wpa_printf(MSG_DEBUG, "nl80211: Generic netlink global init done");
...
static int nl80211_init_connect_handle(struct i802_bss *bss)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Initialize connect event handle for %s",
		   bss->ifname);
	...
	nl80211_register_eloop_read(&bss->nl_connect,
				    wpa_driver_nl80211_event_receive,
				    bss->nl_cb, 1);
	wpa_printf(MSG_DEBUG, "nl80211: Connect event handle ready for %s",
		   bss->ifname);
	return 0;
}
...
{
	wpa_printf(MSG_DEBUG, "nl80211: Initialize BSS object for %s",
		   bss->ifname);
	...
	wpa_printf(MSG_DEBUG, "nl80211: BSS object initialized for %s",
		   bss->ifname);
					  ...
					  const char *driver_params)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Begin interface init for %s",
		   ifname);
	...
	wpa_printf(MSG_DEBUG, "nl80211: Allocate interface state for %s",
		   ifname);
	...
	if (nl80211_init_bss(bss)) {
		wpa_printf(MSG_ERROR, "nl80211: nl80211_init_bss failed for %s",
			   ifname);
		goto failed;
	}

	wpa_printf(MSG_DEBUG, "nl80211: Finish low-level driver init for %s",
		   ifname);

	if (wpa_driver_nl80211_finish_drv_init(drv, set_addr, 1,
					       driver_params)) {
		wpa_printf(MSG_ERROR, "nl80211: finish_drv_init failed for %s",
			   ifname);
		goto failed;
	}

	wpa_printf(MSG_DEBUG, "nl80211: Low-level driver init complete for %s",
		   ifname);
	...
	wpa_printf(MSG_DEBUG, "nl80211: Create EAPOL TX packet socket for %s",
		   ifname);

	drv->eapol_tx_sock = socket(PF_PACKET, SOCK_DGRAM, 0);
	if (drv->eapol_tx_sock < 0) {
		wpa_printf(MSG_ERROR,
			   "nl80211: EAPOL TX socket(PF_PACKET,SOCK_DGRAM) failed: %s",
			   strerror(errno));
		goto failed;
	}
...
skip_wifi_status:
	...
	if (drv->global) {
		wpa_printf(MSG_DEBUG,
			   "nl80211: Refresh global memberships after %s init",
			   ifname);
		nl80211_check_global(drv->global);
	...
	wpa_printf(MSG_DEBUG, "nl80211: Interface init complete for %s",
		   ifname);
...
#undef NAN_PUB_ACTION
	...
	/* S1G Action frames, used by the sim TWT validation path. */
	if (nl80211_register_action_frame(bss, (u8 *) "\x17", 1) < 0)
		ret = -1;
				   ...
				   const char *driver_params)
{
	...
	wpa_printf(MSG_DEBUG,
		   "nl80211: finish_drv_init start ifname=%s ifindex=%d first=%d",
		   bss->ifname, drv->ifindex, first);
	nuttx_hwsim_debugf("hwsim-debug: finish_drv_init ifname=%s ifindex=%d first=%d hostapd=%d\n",
	       bss->ifname, drv->ifindex, first, drv->hostapd);
	...
	nlmode = nl80211_get_ifmode(bss);
	wpa_printf(MSG_DEBUG, "nl80211: initial interface mode for %s is %d",
		   bss->ifname, nlmode);
	nuttx_hwsim_debugf("hwsim-debug: finish_drv_init initial ifmode=%d\n", nlmode);

	if (!bss->if_dynamic && nlmode == NL80211_IFTYPE_AP)
		bss->static_ap = 1;
	...
	wpa_printf(MSG_DEBUG, "nl80211: Query driver capabilities for %s",
		   bss->ifname);
	if (wpa_driver_nl80211_capa(drv)) {
		nuttx_hwsim_debugf("hwsim-debug: finish_drv_init capa failed\n");
		return -1;
	}
	wpa_printf(MSG_DEBUG, "nl80211: Driver capabilities ready for %s",
		   bss->ifname);
	nuttx_hwsim_debugf("hwsim-debug: finish_drv_init capa ok phy=%s flags2=0x%llx\n",
	       drv->phyname, (unsigned long long) drv->capa.flags2);

	if (driver_params && nl80211_set_param(bss, driver_params) < 0) {
		nuttx_hwsim_debugf("hwsim-debug: finish_drv_init set_param failed\n");
		return -1;
	}
	...
	if (first && nlmode == NL80211_IFTYPE_STATION)
	...
	wpa_printf(MSG_DEBUG, "nl80211: target interface mode for %s is %d",
		   bss->ifname, nlmode);
	nuttx_hwsim_debugf("hwsim-debug: finish_drv_init target ifmode=%d static_ap=%d dynamic=%d\n",
	       nlmode, bss->static_ap, bss->if_dynamic);

	wpa_printf(MSG_DEBUG, "nl80211: Set interface mode for %s",
		   bss->ifname);
	if (wpa_driver_nl80211_set_mode(bss, nlmode) < 0) {
		wpa_printf(MSG_ERROR, "nl80211: Could not configure driver mode");
		nuttx_hwsim_debugf("hwsim-debug: finish_drv_init set_mode failed mode=%d\n",
		       nlmode);
		return -1;
	}
	wpa_printf(MSG_DEBUG, "nl80211: Interface mode set for %s",
		   bss->ifname);
	nuttx_hwsim_debugf("hwsim-debug: finish_drv_init set_mode ok mode=%d\n", nlmode);
	...
	if (!rfkill_is_blocked(drv->rfkill)) {
		...
		if (ret) {
			...
			nuttx_hwsim_debugf("hwsim-debug: finish_drv_init iface up failed ret=%d\n",
			       ret);
			return ret;
		}
		wpa_printf(MSG_DEBUG, "nl80211: Interface %s is up",
			   bss->ifname);
		nuttx_hwsim_debugf("hwsim-debug: finish_drv_init iface up ok\n");
	...
	if (nlmode != NL80211_IFTYPE_P2P_DEVICE) {
		wpa_printf(MSG_DEBUG, "nl80211: Read interface MAC for %s",
			   bss->ifname);
		if (linux_get_ifhwaddr(drv->global->ioctl_sock, bss->ifname,
				       bss->addr)) {
			nuttx_hwsim_debugf("hwsim-debug: finish_drv_init get_ifhwaddr failed errno=%d\n",
			       errno);
			return -1;
		}
		os_memcpy(drv->perm_addr, bss->addr, ETH_ALEN);
	}
	wpa_printf(MSG_DEBUG, "nl80211: finish_drv_init done for %s",
		   bss->ifname);
	nuttx_hwsim_debugf("hwsim-debug: finish_drv_init done\n");
				   ...
				   struct wpa_driver_ap_params *params)
{
	...
	case HOSTAPD_MODE_IEEE80211AH:
		band = nla_nest_start(msg, NL80211_BAND_S1GHZ);
		break;
	default:
		return 0;
	}
				     ...
				     struct wpa_driver_ap_params *params)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: ssid=%s",
		   wpa_ssid_txt(params->ssid, params->ssid_len));
	if (!(msg = nl80211_ifindex_msg_build(drv, nlmsg_alloc_size(4096),
					       bss->ifindex, 0, cmd)) ||
				   ...
				   size_t data_len)
{
...
#ifdef __NuttX__
	{
		...
		native_ifindex = ieee80211_linux_if_indextonative(bss->ifindex);
		if (native_ifindex > 0)
			ll.sll_ifindex = native_ifindex;
	}
#endif /* __NuttX__ */
...
static int nl80211_bind_eapol_sock(struct i802_bss *bss)
{
	struct sockaddr_ll ll;
#ifdef __NuttX__
	int native_ifindex;
	...
	if (native_ifindex > 0) {
		wpa_printf(MSG_INFO,
			   "nl80211: EAPOL RX ifindex linux=%d native=%d",
			   bss->ifindex, native_ifindex);
		ll.sll_ifindex = native_ifindex;
		add_ifidx(bss->drv, native_ifindex, IFIDX_ANY);
	}
#endif /* __NuttX__ */
	ll.sll_protocol = htons(ETH_P_PAE);

	if (bind(bss->drv->eapol_sock, (struct sockaddr *) &ll,
		 sizeof(ll)) < 0) {
		wpa_printf(MSG_ERROR,
			   "nl80211: bind EAPOL RX socket to ifindex %d failed: %s",
			   bss->ifindex, strerror(errno));
		return -1;
	}
	wpa_printf(MSG_INFO, "nl80211: EAPOL RX socket bound ifindex=%d",
		   ll.sll_ifindex);
		       ...
		       struct wpa_init_params *params)
{
	...
	nuttx_hwsim_debugf("hwsim-debug: i802_init enter ifname=%s\n", params->ifname);

	bss = wpa_driver_nl80211_drv_init(hapd, params->ifname,
					  params->global_priv, 1,
					  params->bssid, params->driver_params);
	if (bss == NULL) {
		wpa_printf(MSG_ERROR, "nl80211: AP drv_init failed for %s",
			   params->ifname);
		return NULL;
	}

	drv = bss->drv;
	nuttx_hwsim_debugf("hwsim-debug: i802_init drv_init ok ifindex=%d\n",
	       drv->ifindex);
	...
	nuttx_hwsim_debugf("hwsim-debug: i802_init after add_ifidx eapol_sock=%d flags2=0x%llx\n",
	       drv->eapol_sock, (unsigned long long) drv->capa.flags2);
	...
	nuttx_hwsim_debugf("hwsim-debug: i802_init eapol socket ok fd=%d\n",
	       drv->eapol_sock);

	if (nl80211_bind_eapol_sock(bss) < 0)
		goto failed;
	...
	nuttx_hwsim_debugf("hwsim-debug: i802_init eloop register ok\n");
skip_eapol_sock:

	if (linux_get_ifhwaddr(drv->global->ioctl_sock, bss->ifname,
			       params->own_addr)) {
		wpa_printf(MSG_ERROR,
			   "nl80211: linux_get_ifhwaddr failed for AP interface %s: %s",
			   bss->ifname, strerror(errno));
		goto failed;
	}
	nuttx_hwsim_debugf("hwsim-debug: i802_init hwaddr ok %02x:%02x:%02x:%02x:%02x:%02x\n",
	       params->own_addr[0], params->own_addr[1], params->own_addr[2],
	       params->own_addr[3], params->own_addr[4], params->own_addr[5]);
				  ...
				  size_t csa_offs_len, int link_id)
{
	...
	msg_len = 1024 + buf_len + csa_offs_len * sizeof(u16);
	msg = nlmsg_alloc_size(msg_len);
	if (!msg) {
		wpa_printf(MSG_DEBUG, "nl80211: CMD_FRAME alloc failed len=%zu",
			   msg_len);
		goto fail;
	}
	...
	if (!nl80211_cmd(drv, msg, 0, NL80211_CMD_FRAME) ||
	    nl80211_set_iface_id(msg, bss) < 0) {
		wpa_printf(MSG_DEBUG, "nl80211: CMD_FRAME header/ifindex failed");
		goto fail;
	}

	if (link_id != NL80211_DRV_LINK_ID_NA &&
	    nla_put_u8(msg, NL80211_ATTR_MLO_LINK_ID, link_id)) {
		wpa_printf(MSG_DEBUG, "nl80211: CMD_FRAME MLO link attr failed");
		goto fail;
	}
	if (freq && nla_put_u32(msg, NL80211_ATTR_WIPHY_FREQ, freq)) {
		wpa_printf(MSG_DEBUG, "nl80211: CMD_FRAME freq attr failed");
		goto fail;
	}
	if (wait && nla_put_u32(msg, NL80211_ATTR_DURATION, wait)) {
		wpa_printf(MSG_DEBUG, "nl80211: CMD_FRAME duration attr failed");
		goto fail;
	}
	if (offchanok && ((drv->capa.flags & WPA_DRIVER_FLAGS_OFFCHANNEL_TX) ||
			  drv->test_use_roc_tx) &&
	    nla_put_flag(msg, NL80211_ATTR_OFFCHANNEL_TX_OK)) {
		wpa_printf(MSG_DEBUG, "nl80211: CMD_FRAME offchan attr failed");
		goto fail;
	}
	if (no_cck && nla_put_flag(msg, NL80211_ATTR_TX_NO_CCK_RATE)) {
		wpa_printf(MSG_DEBUG, "nl80211: CMD_FRAME no-cck attr failed");
		goto fail;
	}
	if (no_ack && nla_put_flag(msg, NL80211_ATTR_DONT_WAIT_FOR_ACK)) {
		wpa_printf(MSG_DEBUG, "nl80211: CMD_FRAME no-ack attr failed");
		goto fail;
	}
	if (csa_offs && nla_put(msg, NL80211_ATTR_CSA_C_OFFSETS_TX,
				csa_offs_len * sizeof(u16), csa_offs)) {
		wpa_printf(MSG_DEBUG, "nl80211: CMD_FRAME CSA attr failed");
		goto fail;
	}
	if (nla_put(msg, NL80211_ATTR_FRAME, buf_len, buf)) {
		wpa_printf(MSG_DEBUG, "nl80211: CMD_FRAME frame attr failed len=%zu msg_len=%zu",
			   buf_len, msg_len);
		goto fail;
	}

	cookie = 0;
	wpa_printf(MSG_MSGDUMP, "nl80211: CMD_FRAME sending len=%zu msg_len=%zu",
		   buf_len, msg_len);
	ret = no_ack ? send_and_recv_cmd(drv, msg) :
		send_and_recv_resp(drv, msg, cookie_handler, &cookie);
	msg = NULL;
	if (ret) {
...
static void * nl80211_global_init(void *ctx)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Creating ioctl socket");
	...
	wpa_printf(MSG_DEBUG, "nl80211: Global driver init complete");
...
#ifdef __NuttX__
	return ieee80211_linux_set_data_frame_filters(bss->ifname,
						      filter_flags);
#endif /* __NuttX__ */
...
static int nl80211_wnm_copy_tfs_req(struct wpa_driver_nl80211_data *drv,
				    const u8 *buf, u16 len)
{
	u8 *copy;

	if (!buf || len < 2 || len > 1024 || buf[0] != WLAN_EID_TFS_REQ ||
	    buf[1] + 2U > len)
		return -1;

	copy = os_memdup(buf, len);
	if (!copy)
		return -1;

	os_free(drv->wnm_tfs_req_ie);
	drv->wnm_tfs_req_ie = copy;
	drv->wnm_tfs_req_ie_len = len;
	wpa_printf(MSG_DEBUG, "nl80211: WNM stored TFS Request IE len=%u", len);
	return 0;
}


static int nl80211_wnm_build_tfs_resp(struct wpa_driver_nl80211_data *drv,
				      u8 *buf, u16 *buf_len)
{
	if (!buf || !buf_len)
		return -1;

	if (!drv->wnm_tfs_req_ie || drv->wnm_tfs_req_ie_len < 2) {
		*buf_len = 0;
		return 0;
	}

	os_memcpy(buf, drv->wnm_tfs_req_ie, drv->wnm_tfs_req_ie_len);
	buf[0] = WLAN_EID_TFS_RESP;
	*buf_len = drv->wnm_tfs_req_ie_len;
	wpa_hexdump(MSG_DEBUG, "nl80211: WNM generated TFS Response IE",
		    buf, *buf_len);
	return 0;
}
			    ...
			    u8 *buf, u16 *buf_len)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: WNM oper=%d peer=" MACSTR,
		   oper, MAC2STR(peer));
	...
	case WNM_SLEEP_TFS_REQ_IE_SET:
		if (!buf_len)
			return -1;
		return nl80211_wnm_copy_tfs_req(drv, buf, *buf_len);
	case WNM_SLEEP_TFS_RESP_IE_ADD:
		return nl80211_wnm_build_tfs_resp(drv, buf, buf_len);
	...
	case WNM_SLEEP_TFS_RESP_IE_SET:
		if (buf_len)
			wpa_hexdump(MSG_DEBUG, "nl80211: WNM accepted TFS Response IE",
				    buf, *buf_len);
		return 0;
	case WNM_SLEEP_TFS_IE_DEL:
		os_free(drv->wnm_tfs_req_ie);
		drv->wnm_tfs_req_ie = NULL;
		drv->wnm_tfs_req_ie_len = 0;
		return 0;
	default:
		return -1;
	}
