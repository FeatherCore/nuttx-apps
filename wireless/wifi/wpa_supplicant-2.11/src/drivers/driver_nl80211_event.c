					      ...
					      struct nlattr *resp_ie)
{
	...
	if ((mlo->req_links & mlo->valid_links) != mlo->valid_links) {
		wpa_printf(MSG_ERROR,
			   ...
			   (unsigned int) mlo->req_links,
			   (unsigned int) mlo->valid_links,
			   (unsigned int) req_info.non_assoc_links,
			   (unsigned int) BIT(mlo->assoc_link_id));
		return -1;
	}
			      ...
			      struct nlattr *tb[])
{
	...
	if (cqm[NL80211_ATTR_CQM_TXE_RATE] &&
		...
		wpa_printf(MSG_DEBUG, "nl80211: CQM TXE event for " MACSTR
			   ...
			   (unsigned int) nla_get_u32(cqm[NL80211_ATTR_CQM_TXE_RATE]),
			   (unsigned int) nla_get_u32(cqm[NL80211_ATTR_CQM_TXE_PKTS]),
			   (unsigned int) nla_get_u32(cqm[NL80211_ATTR_CQM_TXE_INTVL]));
		return;
	}
				 ...
				 u8 chan, enum hostapd_hw_mode hw_mode)
{
	...
	if (hw_mode == HOSTAPD_MODE_IEEE80211AH) {
		if (chan >= 1 && chan <= 51)
			return 902 + chan / 2;
		return 0;
	}
				   ...
				   struct nlattr *mac, struct nlattr *timeout)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Association comeback requested by "
		   MACSTR " (timeout: %u ms)",
		   MAC2STR((u8 *) nla_data(mac)),
		   (unsigned int) nla_get_u32(timeout));
}
...
#ifdef __NuttX__
	/*
	 * The NuttX generic netlink bridge can expose locally generated
	 * request echoes on the nl_event socket while userspace and the
	 * in-kernel nl80211 port are still sharing a lightweight netlink
	 * transport. These are not Linux nl80211 multicast events, and
	 * treating commands such as SET_WIPHY as driver events only adds
	 * noise and can confuse later event-state diagnostics.
	 */
	if (nlh->nlmsg_flags & NLM_F_REQUEST)
		return NL_SKIP;

	if (gnlh->cmd == NL80211_CMD_SET_WIPHY ||
	    gnlh->cmd == NL80211_CMD_SET_WIPHY_NETNS)
		return NL_SKIP;
#endif /* __NuttX__ */
