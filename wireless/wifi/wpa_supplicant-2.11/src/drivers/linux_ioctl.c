...
int linux_set_iface_flags(int sock, const char *ifname, int dev_up)
{
	...
	if (ioctl(sock, SIOCGIFFLAGS, &ifr) != 0) {
		ret = errno ? -errno : -999;
		ret = ieee80211_linux_set_netdev_flags(ifname, dev_up != 0);
		if (ret != -ENODEV)
			return ret;
		ret = errno ? -errno : -999;
	...
	if (dev_up) {
		if (ifr.ifr_flags & IFF_UP) {
			ret = ieee80211_linux_set_netdev_flags(ifname, true);
			return ret == -ENODEV ? 0 : ret;
		}
		ifr.ifr_flags |= IFF_UP;
	} else {
		if (!(ifr.ifr_flags & IFF_UP)) {
			ret = ieee80211_linux_set_netdev_flags(ifname, false);
			return ret == -ENODEV ? 0 : ret;
		}
		ifr.ifr_flags &= ~IFF_UP;
	}
...
int linux_iface_up(int sock, const char *ifname)
{
	...
	if (ioctl(sock, SIOCGIFFLAGS, &ifr) != 0) {
		ret = errno ? -errno : -999;
		if (ieee80211_linux_if_nametoindex(ifname) > 0)
			return 0;
		wpa_printf(MSG_ERROR, "Could not read interface %s flags: %s",
			   ifname, strerror(errno));
		return ret;
...
int linux_get_ifhwaddr(int sock, const char *ifname, u8 *addr)
{
	...
	if (ioctl(sock, SIOCGIFHWADDR, &ifr)) {
		ret = ieee80211_linux_get_netdev_mac(ifname, addr);
		if (ret == 0) {
			wpa_printf(MSG_DEBUG,
				   "NuttX: got Linux compat interface %s hwaddr",
				   ifname);
			return 0;
		}
...
int linux_set_ifhwaddr(int sock, const char *ifname, const u8 *addr)
{
	...
	if (ioctl(sock, SIOCSIFHWADDR, &ifr)) {
		if (ieee80211_linux_set_netdev_mac(ifname, addr) == 0) {
			wpa_printf(MSG_DEBUG,
				   "NuttX: set Linux compat interface %s hwaddr",
				   ifname);
			return 0;
		}
	...
	if (ieee80211_linux_set_netdev_mac(ifname, addr) == 0)
		wpa_printf(MSG_DEBUG,
			   "NuttX: synced Linux compat interface %s hwaddr",
			   ifname);
