		    ...
		    struct wpa_driver_scan_params *params)
{
	...
	msg = nl80211_cmd_msg_alloc(bss, 0, cmd, 4096);
	if (!msg)
		return NULL;
				  ...
				  struct wpa_driver_scan_params *params)
{
...
#ifdef __NuttX__
	wpa_printf(MSG_DEBUG,
		   "nl80211: sched_scan unsupported on current NuttX nl80211 port");
	...
	if (ret) {
		if (ret == -EOPNOTSUPP) {
			wpa_printf(MSG_DEBUG,
				   "nl80211: sched_scan unsupported at runtime; disabling offload scans");
			drv->capa.sched_scan_supported = 0;
		}
