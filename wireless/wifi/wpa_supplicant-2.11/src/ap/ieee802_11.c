...
static u16 copy_supp_rates(struct hostapd_data *hapd, struct sta_info *sta,
			   struct ieee802_11_elems *elems)
{
	/* Supported rates are not used in IEEE 802.11ad/DMG or 802.11ah/S1G. */
	if (hapd->iface->current_mode &&
	    (hapd->iface->current_mode->mode == HOSTAPD_MODE_IEEE80211AD ||
	     hapd->iface->current_mode->mode == HOSTAPD_MODE_IEEE80211AH))
		return WLAN_STATUS_SUCCESS;
...
#ifdef CONFIG_TESTING_OPTIONS
static int handle_twt_action(struct hostapd_data *hapd,
			     const struct ieee80211_mgmt *mgmt, size_t len)
{
	const u8 *action;
	...
	action = (const u8 *) &mgmt->u.action.u;

	switch (action[0]) {
	case S1G_ACT_TWT_SETUP:
		if (len < IEEE80211_HDRLEN + 3 + 2) {
			wpa_printf(MSG_DEBUG,
				   "TWT: AP setup request too short from "
				   MACSTR " len=%lu",
				   MAC2STR(mgmt->sa), (unsigned long) len);
			return 1;
		}

		ie = (u8 *) action + 2;
		ie_len = len - IEEE80211_HDRLEN - 3;
		if (ie_len < 2 || ie[0] != WLAN_EID_TWT ||
		    ie[1] + 2 > ie_len || ie[1] < 3) {
			wpa_printf(MSG_DEBUG,
				   "TWT: AP setup request missing valid TWT IE from "
				   MACSTR,
				   MAC2STR(mgmt->sa));
			return 1;
		}
		...
		action = (const u8 *) &resp->u.action.u;
		ie = (u8 *) action + 2;
		req_type = WPA_GET_LE16(ie + 3);
		req_type &= ~(BIT(0) | (0x7 << 1));
		req_type |= 4 << 1; /* TWT Setup Command: Accept */
		WPA_PUT_LE16(ie + 3, req_type);

		wpa_printf(MSG_DEBUG,
			   "TWT: AP accept setup from " MACSTR
			   " dialog=%u req_type=0x%04x len=%lu",
			   MAC2STR(mgmt->sa), action[1], req_type,
			   (unsigned long) len);

		ret = hostapd_drv_send_mlme(hapd, resp, len, 0, NULL, 0, 0);
		if (ret < 0)
			wpa_printf(MSG_ERROR,
				   "TWT: AP failed to send setup response");
		os_free(resp);
		return 1;
	case S1G_ACT_TWT_TEARDOWN:
		wpa_printf(MSG_DEBUG, "TWT: AP teardown from " MACSTR
			   " flags=0x%02x",
			   MAC2STR(mgmt->sa),
			   len >= IEEE80211_HDRLEN + 3 ? action[1] : 0);
		return 1;
	default:
		wpa_printf(MSG_DEBUG, "TWT: AP ignored S1G action %u from "
			   MACSTR, action[0], MAC2STR(mgmt->sa));
		return 0;
	}
}
#endif /* CONFIG_TESTING_OPTIONS */
...
#endif /* CONFIG_WNM_AP */
#ifdef CONFIG_TESTING_OPTIONS
	case WLAN_ACTION_S1G:
		return handle_twt_action(hapd, mgmt, len);
#endif /* CONFIG_TESTING_OPTIONS */
#ifdef CONFIG_FST
