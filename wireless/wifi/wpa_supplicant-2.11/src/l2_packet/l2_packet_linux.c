		   ...
		   const u8 *buf, size_t len)
{
	...
	if (l2->l2_hdr) {
...
#ifdef __NuttX__
		{
			...
			native_ifindex = ieee80211_linux_if_indextonative(l2->ifindex);
			if (native_ifindex > 0)
	...
	void *rx_callback_ctx, int l2_hdr)
{
	...
	if (ioctl(l2->fd, SIOCGIFINDEX, &ifr) < 0) {
		ifr.ifr_ifindex = ieee80211_linux_if_nametoindex(l2->ifname);
...
have_ifindex:
...
#ifdef __NuttX__
	{
		...
		native_ifindex = ieee80211_linux_if_indextonative(ifr.ifr_ifindex);
		if (native_ifindex > 0)
			ll.sll_ifindex = native_ifindex;
	}
#endif /* __NuttX__ */
	...
	if (ioctl(l2->fd, SIOCGIFHWADDR, &ifr) < 0) {
		if (ieee80211_linux_get_netdev_mac(l2->ifname,
						   l2->own_addr) == 0)
			goto have_hwaddr;
...
have_hwaddr:
	...
	if (rx_callback) {
		...
		ret = eloop_register_read_sock(l2->fd, l2_packet_receive,
					       l2, NULL);
		if (ret < 0) {
			wpa_printf(MSG_ERROR,
				   "l2_packet: failed to register RX fd=%d ifname=%s proto=0x%04x",
				   l2->fd, l2->ifname, protocol);
			close(l2->fd);
			os_free(l2);
			return NULL;
		}
	...
	void *rx_callback_ctx, int l2_hdr)
{
...
#ifdef __NuttX__
	{
		...
		wpa_printf(MSG_INFO,
			   "l2_packet: bridge bind ifname=%s ifindex linux=%d native=%d proto=0x%04x",
			   l2->ifname, l2->ifindex, ll.sll_ifindex,
			   ETH_P_ALL);
	}
#endif /* __NuttX__ */
