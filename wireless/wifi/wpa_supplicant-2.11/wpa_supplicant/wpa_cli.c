...
#ifdef __NuttX__
static void wpa_cli_reset_for_builtin(void)
{
	optind = 1;
	optarg = NULL;
	optopt = 0;
...
static int wpa_cli_cmd_file_arg(struct wpa_ctrl *ctrl, const char *cmd,
				int min_args, int argc, char *argv[])
{
	...
	file_argv[0] = arg;
	ret = wpa_cli_cmd(ctrl, cmd, min_args, 1, file_argv);
	os_free(arg);
	return ret;
}
...
static int wpa_cli_cmd_dpp_qr_code(struct wpa_ctrl *ctrl, int argc,
				   char *argv[])
{
	return wpa_cli_cmd_file_arg(ctrl, "DPP_QR_CODE", 1, argc, argv);
}
...
static int wpa_cli_cmd_dpp_auth_init(struct wpa_ctrl *ctrl, int argc,
				     char *argv[])
{
	return wpa_cli_cmd_file_arg(ctrl, "DPP_AUTH_INIT", 1, argc, argv);
}
...
int main(int argc, char *argv[])
{
	 ...
	 * wpa_cli command must not reinitialize or destroy the global eloop while
	 * a background wpa_supplicant task is using it.
	 */
