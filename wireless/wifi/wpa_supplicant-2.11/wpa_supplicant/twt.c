...
void wpas_twt_rx_action(struct wpa_supplicant *wpa_s,
			const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	action_len = len - IEEE80211_HDRLEN - 1;

	switch (action[0]) {
	case S1G_ACT_TWT_SETUP:
		if (action_len < 4) {
			wpa_dbg(wpa_s, MSG_DEBUG,
				"TWT: STA setup response too short from " MACSTR
				" len=%lu",
		...
		ie = action + 2;
		ie_len = action_len - 2;
		if (ie_len < 17 || ie[0] != WLAN_EID_TWT ||
		    ie[1] + 2 > ie_len || ie[1] < 15) {
			wpa_dbg(wpa_s, MSG_DEBUG,
				"TWT: STA setup response missing valid TWT IE from "
				MACSTR " ie_len=%lu",
				MAC2STR(mgmt->sa), (unsigned long) ie_len);
			return;
		}

		req_type = WPA_GET_LE16(ie + 3);
		setup_cmd = (req_type >> 1) & 0x7;
		wpa_dbg(wpa_s, MSG_DEBUG,
			"TWT: STA parsed setup response from " MACSTR
			" dialog=%u control=0x%02x req_type=0x%04x "
			"setup_cmd=%u requestor=%u trigger=%u implicit=%u "
			"flow_type=%u flow_id=%u exponent=%u protection=%u "
			"twt=%llu min_twt=%u mantissa=%u channel=%u",
			MAC2STR(mgmt->sa), action[1], ie[2], req_type,
			setup_cmd, !!(req_type & BIT(0)), !!(req_type & BIT(4)),
			!!(req_type & BIT(5)), !!(req_type & BIT(6)),
			(req_type >> 7) & 0x7, (req_type >> 10) & 0x1f,
			!!(req_type & BIT(15)),
			(unsigned long long) WPA_GET_LE64(ie + 5), ie[13],
			WPA_GET_LE16(ie + 14), ie[16]);
		return;
	case S1G_ACT_TWT_TEARDOWN:
		wpa_dbg(wpa_s, MSG_DEBUG,
			"TWT: STA teardown action from " MACSTR " flags=0x%02x",
			MAC2STR(mgmt->sa), action_len >= 2 ? action[1] : 0);
		return;
	default:
		wpa_dbg(wpa_s, MSG_DEBUG,
			"TWT: STA ignored S1G action %u from " MACSTR,
			action[0], MAC2STR(mgmt->sa));
		return;
	}
}
