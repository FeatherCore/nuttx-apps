	...
	if (tb[IFLA_LINKINFO]) {
		...
		err = nla_parse_nested(li, IFLA_INFO_MAX, tb[IFLA_LINKINFO],
			...
			if (li[IFLA_INFO_KIND]) {
				struct rtnl_link_info_ops *info_ops;
			...
			if ((af = nl_str2af(kind)) >= 0 &&
				...
				info_ops = rtnl_link_info_ops_lookup(kind);
				link->l_info_ops = info_ops;

				if (info_ops) {
					if (info_ops->io_parse &&
					    (li[IFLA_INFO_DATA] || li[IFLA_INFO_XSTATS])) {
						err = info_ops->io_parse(link, li[IFLA_INFO_DATA],
								    li[IFLA_INFO_XSTATS]);
						if (err < 0)
							goto errout;
				} else {
