...
int rtnl_link_af_data_compare(struct rtnl_link *a, struct rtnl_link *b,
			      int family)
{
	struct rtnl_link_af_ops *family_ops;
	int ret = 0;
	...
	family_ops = rtnl_link_af_ops_lookup(family);
	if (!family_ops)
		return ~0;

	if (family_ops->ao_compare == NULL) {
		ret = ~0;
		goto out;
	}

	ret = family_ops->ao_compare(a, b, family, ~0, 0);

out:
	rtnl_link_af_ops_put(family_ops);
