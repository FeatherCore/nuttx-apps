...
continue_reading:
	...
	nuttx_hwsim_debugf("hwsim-debug: libnl recv n=%d first_len=%u first_type=%u flags=0x%x seq=%u\n",
	       n, ((struct nlmsghdr *) buf)->nlmsg_len,
	       ((struct nlmsghdr *) buf)->nlmsg_type,
	       ((struct nlmsghdr *) buf)->nlmsg_flags,
	       ((struct nlmsghdr *) buf)->nlmsg_seq);
	NL_DBG(3, "recvmsgs(%p): Read %d bytes\n", sk, n);

	hdr = (struct nlmsghdr *) buf;
	while (nlmsg_ok(hdr, n)) {
		nuttx_hwsim_debugf("hwsim-debug: libnl msg len=%u type=%u flags=0x%x seq=%u remaining=%d\n",
