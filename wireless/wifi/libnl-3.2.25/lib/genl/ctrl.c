...
static int genl_ctrl_registered;

static void genl_ctrl_ensure_init(void)
{
	if (!genl_ctrl_registered) {
		genl_register(&genl_ctrl_ops);
		genl_ctrl_registered = 1;
	}
}
