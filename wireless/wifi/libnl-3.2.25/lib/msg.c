...
#else
		char *errstr = strerror_r(-err->error, buf, sizeof(buf));
#endif
...
static void print_msg(struct nl_msg *msg, FILE *ofd, struct nlmsghdr *hdr)
{
	...
	if (attrlen) {
		...
		attrs_len = nlmsg_attrlen(hdr, ops->co_hdrsize);
		dump_attrs(ofd, attrs, attrs_len, 0);
	}
